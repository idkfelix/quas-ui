<script lang="ts" generics="T">
	...
	let { name, description, props, dataAttrs }: Component<T> = $props();
	const namePrefix = $derived(name.split(".").slice(0, -1).join("."));
	const nameMain = $derived(name.split(".").at(-1));
</script>

<div class="mt-4 mb-2 flex flex-col gap-2 font-medium">
	<code class="flex w-min rounded-lg border bg-card px-3 py-1 text-base">
		<span class="text-muted-foreground">{namePrefix}.</span>
		{nameMain}
	</code>
